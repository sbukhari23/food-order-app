...
export async function GET(
  ...
  const meal = database
    ? await MealModel.findOne({
        $or: [{ _id: mealId }, { slug: mealId }],
      }).lean()
    : meals.find((item) => item.id === mealId);
  if (!meal)
    return NextResponse.json({ error: "Meal not found." }, { status: 404 });
  const reviews = database
    ? await ReviewModel.find({ mealId }).sort({ createdAt: -1 }).lean()
    : [];
...
export async function PATCH(
  ...
  const { mealId } = await params;
  const meal = await MealModel.findByIdAndUpdate(mealId, await request.json(), {
    new: true,
    runValidators: true,
  });
  return meal
...
export async function DELETE(
  ...
  if (!database)
    return NextResponse.json(
      {
        error:
          "We could not remove that meal right now. Please try again shortly.",
      },
      { status: 503 },
    );
  const { mealId } = await params;
