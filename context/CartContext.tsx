...
export type Action =
  | { type: "add"; meal: Meal; quantity?: number }
...
export function cartReducer(state: CartItem[], action: Action): CartItem[] {
    ...
    return action.quantity <= 0
      ...
      : state.map((item) =>
          item.id === action.id
            ? {
                ...item,
                quantity: Math.min(MAX_CART_QUANTITY, action.quantity),
              }
            : item,
        );
  ...
  return [
    ...
    {
      ...action.meal,
      quantity: Math.min(MAX_CART_QUANTITY, action.quantity ?? 1),
    },
  ];
}
...
export function CartProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    queueMicrotask(() => {
      const saved = localStorage.getItem("reactfood-cart");
      if (saved) {
        for (const item of JSON.parse(saved) as CartItem[])
  ...
  useEffect(() => {
    if (hydrated) localStorage.setItem("reactfood-cart", JSON.stringify(items));
  }, [items, hydrated]);
