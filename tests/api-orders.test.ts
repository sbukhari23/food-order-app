...
vi.mock("../models/Meal", () => ({
  MealModel: { findOne: findOneMeal, findById: findByIdMeal },
}));
vi.mock("../models/Order", () => ({
  OrderModel: { create: createOrder, find: findOrders },
}));
...
describe("POST /api/orders", () => {
  ...
  it("uses the database meal price and creates a valid order", async () => {
    const response = await POST(
      request({ items: [{ id: "admin-meal", quantity: 2 }], customer }),
    );
  ...
  it("returns field-specific validation details for missing customer fields", async () => {
    const response = await POST(
      request({ items: [{ id: "admin-meal", quantity: 1 }], customer: {} }),
    );
    const body = await response.json();
  ...
  it("rejects an item that is not in the database", async () => {
    ...
    const response = await POST(
      request({ items: [{ id: "missing-meal", quantity: 1 }], customer }),
    );
    expect(response.status).toBe(400);
  ...
  it("associates a logged-in order and returns it from order history", async () => {
    getSession.mockResolvedValue({
      user: { id: "user-1", role: "user", email: customer.email },
    });
    const response = await POST(
      request({ items: [{ id: "admin-meal", quantity: 1 }], customer }),
    );
    ...
    findOrders.mockReturnValue({
      sort: () => ({ lean: () => Promise.resolve([created.order]) }),
    });
